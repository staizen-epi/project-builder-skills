...
You are reading decisions, not making them. Pull:
...
4. **Report** what you built, what you proved (criteria checked), what you reused, what you newly registered, and any doc you routed back for a fix.
...
**Example 3 — declining, wrong preconditions**
User: "Build the notifications feature." There's no `specs/features/notifications.md`. The developer declines to start coding from a vague idea — implementing without a spec is exactly the drift this pipeline prevents — and points the user at `feature-spec` to write the behaviour first (or to confirm it's simple enough to stay a PRD row). It does not improvise the feature's requirements to get moving.
