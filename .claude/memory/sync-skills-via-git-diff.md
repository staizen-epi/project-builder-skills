...
**Why:** the project (`package/skills/`, committed) is the source of truth; personal (`~/.claude/skills/`) is the reusable mirror, kept in sync manually. Tying syncs to commits keeps the mirror tracking the source predictably instead of relying on folder-diffs of a possibly-dirty working tree. See [[skills-under-test-in-package]].

**How to apply:** to find what to sync, look at git — e.g. `git show --stat <commit>` or `git diff --name-only <last-synced>..HEAD -- package/skills/` — and copy out the skill folders those paths touch. After a commit that changes skills, sync those skills. `rsync -a --delete <skill>/ ~/.claude/skills/<skill>/` mirrors one folder exactly. Edits to existing SKILL.md are picked up live; brand-new skill folders need a Claude Code restart to be watched.
