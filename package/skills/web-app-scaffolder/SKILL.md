...
**Also read `specs/DESIGN.md` if it exists — and adopt it as the foundation's visual language.** A design system may have been established between the PRD/PoC and scaffolding. Unlike `POC-NOTES.md`, DESIGN is a **decision, not a hint**: it names the durable token files and base component inventory the front end is built in. When present, the scaffolder **wires the tokens in and stands up the base components** as part of Phase 0 (see Step 2's "Design system" gated piece) so every feature is built against one coherent visual language. The architecture still wins on *how* components are built (the stack/framework); DESIGN governs *how they look*. If there's no DESIGN.md, scaffold sensible stack-default styling and move on — don't invent a design system here (that's `ux-designer`).
...
- **Design system →** if `specs/DESIGN.md` exists, wire its **token files** into the front end (CSS variables / theme config per the stack) and stand up its **base component inventory** in their defined states, plus the layout/nav shell it specifies. Keep the static style-guide preview page (or regenerate it) so the system stays visible. Conform to its accessibility baseline. The architecture wins on *how* components are built; DESIGN governs *how they look*. No DESIGN.md → use stack-default styling; don't invent a system (route to `ux-designer`).
